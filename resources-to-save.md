# Resources, Shortcuts, and Cheat Sheets:

## Code Editors  
### If you have no idea which one to pick  
* [Blog post with short intro of all three: VS Code, Atom and Vim](https://www.codementor.io/mattgoldspink/best-text-editor-atom-sublime-vim-visual-studio-code-du10872i7)
### Atom  
* [Download Atom](https://atom.io/)  
* [Atom Shortcuts Cheat Sheet](http://d2wy8f7a9ursnm.cloudfront.net/atom-editor-cheat-sheet.pdf)  
* [More Atom Shortcuts](https://github.com/turingschool/bootstrap_new_students/blob/master/cheetsheets/basic_shortcuts.txt)  
* [Video on Customizing Atom](https://www.youtube.com/watch?v=U5POoGSrtGg)  
* [Atom Flight Manual](https://atom.io/docs/latest)  
### VS Code  
* [Download VS Code](https://code.visualstudio.com/)  
* [VS Code intro videos](https://code.visualstudio.com/docs/getstarted/introvideos)  
* [Language Support in VS Code](https://code.visualstudio.com/docs/extensionAPI/language-support)  
### Vim  
* [Download Vim](https://vim.sourceforge.io/download.php)  
* [Setting up Vim from scratch](http://marcgg.com/blog/2016/03/01/vimrc-example/)  
* [Interactive Vim tutorial](http://www.openvim.com/tutorial.html)  
* [Vim extensions](http://vimawesome.com/)  

## Terminal:  
* [Terminal/ Command Line Tips](http://cli.learncodethehardway.org/bash_cheat_sheet.pdf)  
* [More Terminal/ Command Line Tips](https://github.com/turingschool/bootstrap_new_students/blob/master/cheetsheets/terminal.txt)  

## If you have always been a Windows user...  
* [Official Apple "New to Mac" intro](https://support.apple.com/explore/new-to-mac)  
* [Try out MacOS in your browser](http://www.alessioatzeni.com/mac-osx-lion-css3/)  
* [MacOS shortcuts](https://support.apple.com/en-us/HT201236)
